              ...
              xml.Query "select DISTINCT(customer.id),customer.name 
                                ...
                                WHERE So.statusId IN (20,25) 
                                  ...
                                  AND (     (customer.name LIKE '%TOOST%')
                                        OR  (customer.name LIKE '%Generation Records%')
                                        OR  (customer.name LIKE '%Turntable Lab%')
                                        OR  (customer.name LIKE '%A Day In The Life%')
                                        OR  (customer.name LIKE '%Jared Kusmit%')
                                        OR  (customer.name LIKE '%Yellow Dog%')
                                        OR  (customer.name LIKE '%Shangri-La%')
                                        OR  (customer.name LIKE '%Keene On Vinyl%')
                                        OR  (customer.name LIKE '%Kops%')
                                        OR  (customer.name LIKE '%Culture Clash%')
                                        OR  (customer.name LIKE '%RPM%')
                                        OR  (customer.name LIKE '%Rushhour%')
                                        OR  (customer.name LIKE '%Josey%')
                                        OR  (customer.name LIKE '%Academy%')
                                        OR  (customer.name LIKE '%Spinnaker%')
                                        OR  (customer.name LIKE '%Park Ave%')
                                        OR  (customer.name LIKE '%Euclid%')
                                        OR  (customer.name LIKE '%Edsawesome%')
                                        OR  (customer.name LIKE '%Resurrection%')
                                        OR  (customer.name LIKE '%Cobraside%')
                                        OR  (customer.name LIKE '%Tsunami%'))"
            }
          }
        end
